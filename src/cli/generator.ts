import { ResourceModule, SerializeResourceModule, SerializeResourceModuleAsVariable } from "@resmod/webpack/loader/types";
import { CommandLineOptions } from "@resmod/cli/dts";
import { dirname, parse, basename, resolve, relative, join } from "path";
...
export abstract class DTSGenerator {
   ...
   protected commitInternal(dtsMeta: DTSMeta): GeneratedResult {
      ...
      let configFile = resolve("tsconfig.json");
      if (existsSync(configFile)) {
         let tsconfig = require(configFile);
         if (tsconfig.compilerOptions &&
            tsconfig.compilerOptions.declaration &&
            tsconfig.compilerOptions.declarationDir) {

            let rootOutputDir = tsconfig.compilerOptions.declarationDir;
            let file = `${join(rootOutputDir, moduleName.substring(moduleName.indexOf("/") + 1))}.d.ts`;
